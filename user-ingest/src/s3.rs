...
pub async fn upload_to_s3(local_path: &PathBuf, remote_path: &str) -> Result<u16, ProcessError> {
    let file = tokio::fs::File::open(local_path)
        .await
        .map_err(ProcessError::Io)?;
    let mut stream = io::BufReader::new(file);
    let response = S3_BUCKET
        .put_object_stream(&mut stream, remote_path)
        .await
        .map_err(ProcessError::S3)?;
    Ok(response.status_code())
}

pub async fn has_file(remote_path: &str) -> Result<bool, S3Error> {
    S3_BUCKET
